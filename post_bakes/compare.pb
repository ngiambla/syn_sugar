...
Abstract— 
Some modern high%%dsh%%level synthesis (HLS) tools [1] 
...
a program’s arrays should be automatically partitioned into subarrays%%lst%% which are %%cmp_t%% implemented in separate on%%dsh%%chip RAM 
...
where each partition is %%cmp_t%% implemented in separate RAMs 
...
on the ports of RAMs holding the arrays%%lst%% even %%cmp_if%% the RAMs 
...
access a single array%%per%%]$~ ~$[When synthesized to hardware%%lst%% contention ]^[ delays due to arbitration may arise %%cmp_b%% the 
...
locally scoped ]^[ %%cmp_tf%%%%lst%% can be shared between multiple 
...
value greater %%cmp_ta%% 1 ]^[ less %%cmp_ta%% S number of partitions ]^[ 
our block size must be a power of 2 value greater %%cmp_ta%% 1 ]^[ 
less %%cmp_ta%% S%%lst%% where S is the size of the array in the dimension 
of partitioning%%per%%]$~ ~$[In cyclic partitioning%%lst%% we need a power of 2 
number of partitions which is greater %%cmp_ta%% 1 ]^[ less %%cmp_ta%% S 
]^[ a block size of 1%%per%%]$~ ~$[In block cyclic partitioning%%lst%% we must 
have a number of partitions ]^[ block size which are both 
power%%dsh%%of%%dsh%%2 integers greater %%cmp_ta%% 1 ]^[ less %%cmp_ta%% S%%per%%]$~ ~$[Section V%%dsh%%D 
...
using LegUp HLS%%per%%]$~ ~$[The generated RTL is %%cmp_t%% simulated with 
...
the distance will measure 1 cycle%%lst%% %%cmp_b%% the distance captures 
...
arbiter instantiated by LegUp ]^[ %%cmp_tf%%%%lst%% it is able to assess 
...
that ﬁnds the largest power%%dsh%%of%%dsh%%2 number less %%cmp_ta%% the input%%per%%]$~ 
...
index i deﬁnes the legal block sizes ]f[ block cyclic partitioning%%cln%% the block sizes are 2i %%lst%% ]^[ %%cmp_tf%% range from 21 to 
...
to perform worse %%cmp_ta%% other schemes%%per%%]$~ ~$[Large memory traces 
...
stands ]f[ complete%%lst%% block%%lst%% cyclic%%lst%% ]v[ block cyclic%%per%%]$~ ~$[For example%%lst%% %%cmp_if%% we wanted to partition a M × N matrix into 2 blocks%%lst%% 
...
%%cmp_t%% the partitioning scheme would be 0b2 which stands ]f[ 
...
there is%%lst%% %%cmp_t%% that coordinate location is ]n[ within the line of 
...
in Section VI%%dsh%%B to see %%cmp_if%% i) the optimal partitioning schemes 
were detected ]^[ %%cmp_if%% ii) the predicted cycle of last memory 
...
partitions is equal ]v[ less %%cmp_ta%% 16%%lst%% as well as the partition 
