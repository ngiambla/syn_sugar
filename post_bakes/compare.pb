...
in uence result in a higher correlation (assortative mixing) on certain traits between connected %%cmp_ta%% 
...
the network %%cmp_b%% friends could have a higher common latent propensity ]f[ it ]^[ adopt it to a 
larger extent %%cmp_ta%% non%%dsh%%friends%%per%%]$~ ~$[Distinguishing between these factors as the main causes of assorta%%dsh%% 
...
considered one of the most important foci of homophily%%lst%% simply put%%lst%% %%cmp_b%% we are more likely to 
...
induce larger homophily between individuals %%cmp_ta%% it would be the case %%cmp_if%% the ties were random 
...
friend teenagers in uence each other ]^[ %%cmp_h%% become similar on a value homophily level? 
...
of the tendencies in particular%%lst%% we %%cmp_t%% point that out%%per%%]$~ ~$[In order to analyze semantic homophily%%lst%% we 
...
%%cmp%%d to semantic similarity [Harispe et al%%per%% 2015] since in addition to similarity%%lst%% it includes 
...
concepts (abstract ideas in the text%%cln%% ]f[ example%%lst%% %%cmp_if%% an article mentions CERN ]^[ the Higgs 
boson%%lst%% it will have Large Hadron Collider as a concept even %%cmp_if%% the term is ]n[ mentioned explicitly 
...
nication activity (if looking at out%%dsh%%degree)%%per%%]$~ ~$[To sum up%%lst%% %%cmp_ta%%ks to our network being directed ]^[ 
...
network is found to be around 22% [Kwak et al%%per%% 2010] which is lower %%cmp%%d to the other social 
...
addition%%lst%% we identify the aspects of homophily that are speci c ]f[ communication%%lst%% %%cmp%%d to 
...
due to status %%cmp_ta%% to value heterophily%%per%%]$~ ~$[Finally%%lst%% the analysis on the community structure of the 
...
(disconnection)%%lst%% %%cmp_ta%%ks to the type of the network we analyze%%per%%]$~ ~$[Therefore%%lst%% our work offers an ad%%dsh%% 
...
edge ei j = (ui,u j) E %%cmp_if%% a user ui mentions u j%%lst%% ]^[ the edge is assigned the weight wi j = 
...
where Lcomm(SRth) is the number of links in communication network with SR value higher %%cmp_ta%% 
...
look at mutual edges%%lst%% %%cmp_t%% we tackle strong communication ties%%lst%% ]^[ when including all edges%%lst%% %%cmp_t%% 
...
lated%%lst%% as introduced in [Piraveenan et al%%per%% 2012]%%per%%]$~ ~$[These four types of assortativity coef cients show %%cmp_if%% 
the degree of a source node is correlated with the degree of the target nodes%%lst%% %%cmp_h%% tackling relational 
...
other social networks%%lst%% %%cmp_b%% of its role as an information network%%lst%% too%%per%%]$~ ~$[Below we interpret the 
...
network in%%dsh%%in coef cient) become positive%%per%%]$~ ~$[With the threshold larger %%cmp_ta%% 20 mentions%%lst%% the networks 
...
et al%%per%% 2014]%%lst%% we argue that at higher communication intensity (requiring more time ]^[ effort %%cmp_ta%% 
other interactions%%lst%% such as following) the Twitter mention network serves more of a social %%cmp_ta%% 
...
communication network is importantly different %%cmp%%d to the network including also one%%dsh%% 
...
forms of capital%%per%%]$~ ~$[Whether the users who are richer in terms of social capital (and %%cmp_h%% more network 
...
in the Twitter communication network resemble more of the bloggers %%cmp_ta%% the scientists network 
...
lar to the value found in the bloggers network ]^[ lower %%cmp%%d to the scientists network in [Roth 
]^[ Cointet 2010]%%per%%]$~ 
...
%%cmp%%d to popularity ]^[ weaker contacts%%per%%]$~ 
~$[A:16 
S%%per%%]$~ ~$[S cepanovi c et al%%per%%]$~ 
...
of users mentioning considerably more popular users %%cmp_ta%% themselves (leftmost part of the distri%%dsh%% 
...
users discussed above%%lst%% we nd that they tend to be semantically richer %%cmp%%d to the receiving 
...
tween two users tends to increase with status inconsistency of one ]v[ both of the users%%lst%% %%cmp_if%% they are 
both richer on the same form of capital%%per%%]$~ ~$[If the users are status inconsistent ]b[ being rich on differ%%dsh%% 
ent forms of capital%%lst%% %%cmp_t%% their communication intensity tends to decrease%%per%%]$~ ~$[As with other ndings 
...
by communication (and %%cmp_h%% social in uence) ]^[ those that are less likely to be affected (no com%%dsh%% 
...
~$[In particular%%lst%% we %%cmp%% the distributions ]f[ June ]^[ ]f[ October%%per%%]$~ ~$[For communication network%%lst%% 
K%%dsh%%S results in p < e 24 and%%lst%% respectively%%lst%% ]f[ SR network%%lst%% in p < e 197%%lst%% %%cmp_h%% in both cases strongly 
...
even %%cmp_if%% they never communicated%%per%%]$~ ~$[However%%lst%% once again%%lst%% we can ]n[ assert whether the increase is 
...
activated %%cmp_ta%% decommissioned%%per%%]$~ 
...
one aspect where our approach is advantageous %%cmp%%d to the previous approaches%%lst%% that consider 
a formal edge formation (adding someone as a friend ]v[ following) ]^[ do ]n[ require an active user 
engagement afterwards%%per%%]$~ 
~$[Indeed%%lst%% we nd in total 8,166 links that are activated ]^[ %%cmp_t%% also decommissioned during 
our dataset period%%per%%]$~ ~$[The SR change ]f[ such links%%lst%% that are activated ]^[ %%cmp_t%% decommissioned%%lst%% as 
...
higher %%cmp%%d to the whole network%%per%%]$~ ~$[The stability of SR ]f[ an established communication could 
...
we rst notice the difference on persisting links %%cmp%%d to other types of links (and also to the 
...
time%%lst%% have slightly%%lst%% ]b[ notably higher relative social status %%cmp%%d to persisting links%%per%%]$~ ~$[Hence%%lst%% we 
nd evidence that link dissolution happens due to dissimilarity in social status%%per%%]$~ ~$[Another interesting 
observation is that user pairs that start with higher relative status %%cmp%%d to persisting also get 
...
time when link dissolution happens %%cmp%%d to those who consistently persist communication) ]^[ 
ii) higher status differences (also %%cmp%%d to persisting links)%%per%%]$~ ~$[Hence%%lst%% there is indication in Twit%%dsh%% 
ter network that persisting communication links dissolve in the presence of status level heterophily 
rather %%cmp_ta%% value level heterophily%%per%%]$~ 
...
%%cmp%%d to average of persisting links (other results ]f[ weak contacts are similar to strong ]s[ we do 
...
contacts drops%%lst%% ]b[ still stays higher %%cmp_ta%% would be expected after the decommission (existing link is 
...
can be de ned as groups of users that are more tightly connected within the group %%cmp%%d to the 
...
structure of social networks%%lst%% %%cmp_t%% foci can be seen as one such common function ]v[ role around 
...
have higher %%cmp_ta%% average tweets per user ratio%%lst%% which is related to higher reciprocity%%lst%% ]^[ in turn a 
...
]b[ %%cmp_if%% instead they were simply a result of semantically related users connecting more often%%lst%% %%cmp_t%% we 
...
community detection ]^[ the rationale why it performs better on the semantic layer is %%cmp_b%% SR x 
...
~$[Best matching scores ]f[ biggest communities (with more %%cmp_ta%% 50 users) are presented in Table 
...
algorithm BigClam [Yang ]^[ Leskovec 2013] %%cmp_b%% it detects overlapping communities as the 
...
%%cmp_ta%% active (see Fig%%per%% 16%%lst%% right)%%per%%]$~ ~$[Interestingly%%lst%% the same holds ]f[ semantic capital%%cln%% nodes with higher 
community membership are no more likely to be semantically rich %%cmp_ta%% those belonging to less 
...
~$[However%%lst%% as they are ]n[ semantically richer%%lst%% %%cmp_t%% our next assumptions is that such nodes must be 
...
tion intensity decreases %%cmp%%d to average%%per%%]$~ 
...
%%cmp%%d to differences on a value homophily level%%per%%]$~ 
...
a tendency of people who are connected around one foci to nd ]v[ create new foci to streng%%cmp_t%% the 
interaction%%per%%]$~ 
...
social %%cmp_ta%% information network%%per%%]$~ 
...
social reinforcement in uences communication between users ]^[ %%cmp_if%% there exists happiness paradox 
...
on the large amount of knowledge they provide%%per%%]$~ ~$[We %%cmp_t%% apply an open%%dsh%%source script wikiextractor 
...
also gratefully acknowledge the CyberTrust research project ]f[ their support%%per%%]$~ ~$[B%%per%%G%%per%% %%cmp_ta%%ks the Moore ]^[ Sloan Foundations 
Semantic homophily in online communication%%cln%% evidence from Twitter 
A:33 
...
0%%per%%44 
0%%per%%58 
]f[ support as part of the Moore%%dsh%%Sloan Data Science Environment at New York University%%per%%]$~ ~$[P%%per%%H%%per%% %%cmp_ta%%ks General Research 
Fund 26211515 from the Research Grants Council of Hong Kong%%per%%]$~ ~$[S%%per%%]$~ ~$[S%%per%% %%cmp_ta%%ks A%%per%%]$~ ~$[Ukkonen ]f[ the help with the SR database 
...
similar type of SR analysis on Twitter data%%per%%]$~ ~$[The authors also %%cmp_ta%%k A%%per%%]$~ ~$[Gionis ]f[ the helpful discussion ]^[ ]f[ reviewing the 
manuscript%%per%%]$~ 
~$[REFERENCES 
