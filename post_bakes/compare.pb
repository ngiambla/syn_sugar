...
%%cmp%% end—users do not distinguish among sources of 
system failure%%per%%]$~ 
...
%%cmp%%ying its code segmentl%%per%%]$~ ~$[It is possible for an address 
...
long%%lst%% for example%%lst%% %%cmp%% of an inﬁnite loop%%per%%]$~ ~$[Trusted 
...
heads higher %%cmp%% predicted%%lst%% it does not account for 
...
benchmarks is 2%%per%%5%%%lst%% %%cmp%%d to a mean of 5%%per%%6% for 
...
modify a preserved register %%cmp%% it does not need to be 
...
(or rejected %%cmp%% it is not safe)%%lst%% ]^[ %%cmp%% executed at near 
