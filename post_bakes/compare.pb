...
experiments that indicate a much higher %%cmp%% normal operation 
...
%%cmp%% we are to be responsible ]^[ seek the truth%%lst%% we must persevere%%per%%]$~ ~$[I am 
...
heard other such convenient explanations%%lst%% such as%%cln%% %%dqt%%Heavier %%cmp%% air 
machines will never fly%%dqt%% (Lord Kelvin%%lst%% 1890s)%%scn%% %%dqt%%We threw out all of the 
textbooks%%lst%% ]^[ %%cmp%% we built the airplane%%dqt%% (the Wright Brothers)%%scn%% %%dqt%%The 
human body will not survive speeds of over 40 miles per hour%%dqt%% (early 
1900s)%%scn%% ]^[ %%dqt%%Airplanes can not go faster %%cmp%% the speed of sound%%dqt%% 
...
signal in all the noise%%qsn%%]$~ ~$[There is %%cmp%% Tesla%%lst%% Moray%%lst%% ]^[ Bearden are 
correct%%per%%]$~ ~$[The potential payoff is enormous%%lst%% so the search should be 
continued %%cmp%% there is any chance at all of success%%per%%%%dqt%% 
...
is to find It%%lst%% ]^[ %%cmp%% sell It%%lst%% ]^[ finally to deploy It%%per%%%%dqt%% 
...
different %%cmp%% is advertised nationally to the American people%%per%% %%dqt%%The 
...
suppress information %%dsh%% %%cmp%% there is no government%%per%%]$~ ~$[I see instead a 
...
actively suppressed%%lst%% %%cmp%% what are the other possible ways of obtaining 
...
questions were reported to be trick questions%%per%%]$~ ~$[Well%%lst%% %%cmp%% this method can 
...
affiliated with America West Publishers%%lst%% %%cmp%% in southern California%%lst%% 
...
received personal knowledge%%per%%]$~ ~$[I do not know %%cmp%% they have received any 
...
suggested%%cln%% that %%cmp%% we told you that%%lst%% %%cmp%% %%dqt%%they%%dqt%% would kill our 
...
become in touch with that mission%%lst%% ]^[ %%cmp%% to perform that mission%%per%%]$~ 
...
reactions that have a lower chemical ground state %%cmp%% the reactants%%per%%]$~ 
~$[Chemical reactions are caused by electron bonding where electron orbits 
are finally filled ]^[ %%cmp%% higher orbits can be filled%%per%%]$~ ~$[Atomic energy 
...
nearly every day%%lst%% shouldn%%sqt%%t we be interested%%qsn%%]$~ ~$[Well%%lst%% %%cmp%% it does not 
...
~$[This is all very interesting to me%%lst%% %%cmp%% it allows for the 
...
disturbing to some people %%cmp%% the usual lights in the sky%%per%%]$~ ~$[The 
...
in order to %%cmp%% to Meier%%sqt%%s craft photographs%%lst%% ]^[ %%cmp%% the photos 
...
computer analyses ]^[ enhancements%%scn%% ]^[ %%cmp%% read the book by Gary 
...
visited by more %%cmp%% 50 alien cultures on the physical plane%%lst%% all 
...
~$[Rather %%cmp%% discuss them here%%lst%% I will list them in the references%%per%%]$~ ~$[They 
...
the energy transfer process is always less %%cmp%% 100 percent%%per%%]$~ ~$[The 
process (you can envision a box) will always give less energy out %%cmp%% 
energy goes in%%per%%]$~ ~$[The ratio of %%dqt%%energy out%%dqt%% to %%dqt%%energy in%%dqt%% is always less 
%%cmp%% one%%cln%% under%%dsh%%unity%%per%%]$~ ~$[An over%%dsh%%unity device cannot exist%%per%%]$~ ~$[Perpetual 
...
the battery %%cmp%% the engine converts the potential energy within the 
fuel during combustion into crankshaft motion%%lst%% which is %%cmp%% used to 
...
Energy Under%%dsh%%unity%%dqt%% would be when I get more energy out %%cmp%% before ]^[ 
...
unity%%dqt%% would be where I seem to get more energy out %%cmp%% I know am 
...
directed energy from outer space%%per%%]$~ ~$[Just %%cmp%% we can%%sqt%%t measure it%%lst%% 
...
this paper%%lst%% a Free Energy device will appear to produce more energy %%cmp%% 
it uses%%lst%% %%cmp%% it is not known from what form of potential energy 
...
violation of conservation of energy%%lst%% ]b[ %%cmp%% the zero%%dsh%%point energy really 
...
wave energy into electrogravitional potential energy%%lst%% %%cmp%% how to 
...
structure of the scalar potential %%cmp%% becomes a new ]^[ universal 
...
determine %%cmp%% the Earth%%sqt%%s electrostatic (atmospheric) Schumann Cavity can 
be resonated%%lst%% %%cmp%% the power that is delivered to the cavity propagates 
with very low losses%%lst%% ]^[ %%cmp%% power can be extracted at other locations 
...
important %%cmp%% it also describes Tesla%%sqt%%s %%dqt%%Coil for Electro%%dsh%% 
Magnetics,%%dqt%% patented in 1893%%per%%]$~ ~$[This coil looks exactly the same as would 
the counter%%dsh%%wound conical coils of Walter Russell%%lst%% %%cmp%% Russell%%sqt%%s coils 
...
]^[ %%cmp%% periodically ]^[ uniformly re%%dsh%%exciting the circuit as an 
...
he expects to produce 150% ]v[ more energy %%cmp%% input into his motor%%dsh%% 
...
of the sea%%lst%% more in the daytime %%cmp%% at night%%per%%]$~ ~$[At this time Moray had 
...
up ]^[ connected to the ground%%lst%% priming ]^[ %%cmp%% tuning%%lst%% the device 
...
is about 25%%%lst%% which is two to three times greater %%cmp%% the 6 to 8% 
...
orders of magnitude greater %%cmp%% chemical energy%%per%%%%dqt%% Specifically Paul 
...
device in the 1930s%%per%%]$~ ~$[I wonder %%cmp%% charged particles ]v[ charged ions play 
...
dated in 1961%%lst%% the device worked%%cln%% %%dqt%%%%cmp%% of the false concept of 
...
%%dqt%%greater%%lst%% safer power %%cmp%% atomic energy%%dqt%% could be provided for industry 
...
%%cmp%% these super powerful magnets carry enormous ampere pressures 
...
exhibit power transfers much higher %%cmp%% the power required to turn the 
...
battery to operate for a much longer period of time %%cmp%% it normally 
...
~$[It is %%cmp%% conceivable that we are tapping a new energy source%%per%%%%dqt%% 
...
been excluded from the simplified EM equations (%%cmp%%ks to the 
...
~$[The Primordial Energy Field is %%cmp%% also distorted as a consequence of 
...
electrical energy with significantly less mechanical power input %%cmp%% 
...
its publication with all of its consequences will %%cmp%% be yours only%%per%%]$~ 
~$[May peace be with you%%per%%]$~ ~$[Methernitha%%per%%%%dqt%% 
>From the videotape (or the transcription published in 1989)%%lst%% Methernitha 
states%%cln%% 
%%dqt%%This wondermachine is lurked from nature%%lst%% nothing %%cmp%%%%per%%]$~ ~$[Nature is the 
...
%%cmp%% he investigated ]^[ discovered a truth which seemed unacceptable 
by the established science of the days%%per%%%%dqt%% %%dqt%%The book of knowledge is not 
wrong%%lst%% ]b[ it is incomplete%%lst%% ]^[ %%cmp%% allows to draw wrong 
...
important ]v[ better to research %%cmp%% the others listed in this paper%%per%%]$~ 
...
~$[Acknowledgments 
I %%cmp%%k all of the researchers ]^[ authors that have promoted these 
ideas ]^[ have placed themselves on the line in the face of so%%dsh%%called 
adversity%%per%%]$~ ~$[I also %%cmp%%k the staff ]^[ promoters of the IANS for 
conducting this conference%%lst%% ]^[ I %%cmp%%k Tom Bearden for allowing me to 
take his place during this conference in his absence%%per%%]$~ ~$[Finally%%lst%% I %%cmp%%k 
...
companies ]^[ the government backing these establishments do %%cmp%% we can 
...
~$[See %%cmp%% you recognize them%%cln%% Exxon%%lst%% Gulf Oil%%lst%% General Electric%%lst%% 
...
%%cmp%% we are rapidly depleting not only our natural resources ]b[ our 
air ]^[ water%%per%%]$~ ~$[All %%cmp%% of burning fossil fuels%%per%%]$~ ~$[We fund our 
...
might succeed%%per%%]$~ ~$[What would our government do %%cmp%% all the American people 
...
%%cmp%% forever is a long time%%per%%]$~ 
...
discuss their invention with me ]v[ others %%cmp%% they might lose it to 
us ]v[ we might tell someone %%cmp%% before they got it onto the market%%per%%]$~ 
~$[Believe me%%lst%% it won%%sqt%%t get there by going through the patent process%%per%%]$~ ~$[It 
is my feeling that %%cmp%% such a device were introduced at this level%%lst%% %%cmp%% 
...
to commit financial suicide%%per%%]$~ ~$[So what better way %%cmp%% to brand something 
as a secret%%qsn%%]$~ 
...
improper disclosure%%per%%]$~ ~$[However%%lst%% %%cmp%% such part of the subject matter was 
