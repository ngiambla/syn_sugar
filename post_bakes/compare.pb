...
rapidly%%lst%% at a higher level of abstraction %%cmp_ta%% traditional RTL%%per%%]$~ ~$[For 
...
focused on power benefits rather %%cmp_ta%% performance%%per%% 
...
has more %%cmp_ta%% one call point%%per%%]$~ ~$[Additionally%%lst%% a master interface 
...
register samples the asynchronous input signal ]^[ %%cmp_t%% waits 
...
~$[Specifically%%lst%% %%cmp_if%% a start signal is issued by a master interface ]^[ multiple%%dsh%%bit%%dsh%%wide data%%lst%% they cannot be crossed by using a 
...
We %%cmp%% multi%%dsh%%clock designs to single%%dsh%%clock designs%%lst%% wall%%dsh%%clock time%%per%%]$~ ~$[Ratios larger %%cmp_ta%% 1 indicate a “win” ]f[ 
...
correctness%%per%%]$~ ~$[We verified the multi%%dsh%%domain circuits functioned single%%dsh%%clock design%%lst%% ]^[ %%cmp_h%%%%lst%% we use 1 as the ratio ]f[ these 
...
larger %%cmp_ta%% in the inlined case%%lst%% increasing CDC overheads ]^[ 
...
to a single function (and %%cmp_h%% a single domain)%%per%%]$~ 
...
]f[ clock%%dsh%%domain assignment%%lst%% rather %%cmp_ta%% altering the programs 
