...
rst present de nitions of Smart Cities and%%lst%% then%%lst%% discuss the most adopted enabling 
...
analysed using an accident prediction model ]^[ if it indicates an accident%%lst%% the appli%%dsh%% 
...
~$[Veracity%%cln%% because of the large amount of data collected%%lst%% ]^[ the use of multiple data 
sources%%lst%% it is important to ensure data quality%%lst%% because errors in the data ]v[ the usage 
...
2014%%scn%% Aazam et al%%per%% 2014]%%lst%% because a cloud environment is an ideal infrastructure to 
...
has implanted more than 20,000 sensors in the city%%per%%]$~ 
~$[Software Platforms ]f[ Smart Cities 
A:9 
Padova Smart City [Zanella et al%%per%% 2014] uses IoT to create a sensor network in the 
city of Padova%%lst%% Italy%%per%%]$~ ~$[Using more than three hundred sensors%%lst%% the platform collects 
...
emergency services if the patient has an urgent problem%%per%%]$~ ~$[Bressan et al%%per%% [Bressan et al%%per%% 
...
explicitly states so%%lst%% ]v[ if the platform has a component ]v[ module that clearly ful lls 
...
sensors%%lst%% ]^[ SmartSantander [Sanchez et al%%per%% 2014]%%lst%% with more than 20000 sensors%%per%%]$~ 
...
there were more than 20,000 sensors%%lst%% in a city of 178,000 inhabitants collecting a 
large amount of city data [Sanchez et al%%per%% 2014]%%scn%% CiDAP collected more than 50 GBs of 
...
valuable because one cannot know what services a city will need%%per%%]$~ ~$[Scallop4SC [Taka%%dsh%% 
...
because of the huge amount of con gurations needed in a Smart City platform%%per%%]$~ ~$[Other 
A:18 
Santana%%lst%% E%%per%%]$~ ~$[F%%per%%]$~ ~$[Z%%per%% et al%%per%% 
...
2014%%scn%% Perera et al%%per%% 2014]%%per%%]$~ ~$[Data Analysis is also a challenge%%lst%% because it is hard to 
...
~$[Software Platforms ]f[ Smart Cities 
A:19 
Heterogeneity%%cln%% This is a challenge because of the differences between the devices 
...
ergy management in a Smart City health care application is also important%%lst%% because 
...
all the city%%lst%% generating more than 8 GB of data every day%%per%%]$~ 
...
A:20 
Santana%%lst%% E%%per%%]$~ ~$[F%%per%%]$~ ~$[Z%%per%% et al%%per%% 
addressing coordination issues in the sensor nodes can be a problem%%lst%% again because 
...
~$[Finally%%lst%% we compare our proposal with the other two architectures%%per%% 
5%%per%%1%%per%%]$~ ~$[CiDAP 
...
the platform%%lst%% which is important because of the massive amount of data collected in a 
...
~$[CiDAP is mainly concerned with storing ]^[ processing a large amount of data in 
the platform%%per%%]$~ ~$[It is important because of the huge amount of data collected in a city%%per%%]$~ 
...
databases can be more suitable than relational databases%%per%%]$~ 
...
because of the huge amount of devices%%lst%% data%%lst%% ]^[ services in the platform%%per%%]$~ ~$[Privacy ]^[ 
Security are important because the platform collects%%lst%% stores%%lst%% ]^[ processes sensible 
...
which is important because social networks connect citizens%%lst%% the city administration%%lst%% 
...
ture is ensuring the privacy of user data%%lst%% because of the amount of personal ]^[ critical 
data that a platform needs to handle%%lst%% such as user locations ]^[ medical records%%per%%]$~ 
~$[The second most cited challenge is heterogeneity%%lst%% because of the large number of 
different systems%%lst%% services%%lst%% applications%%lst%% ]^[ devices that a platform must support%%per%%]$~ 
~$[We were surprised that only three authors cited scalability as a problem%%lst%% because it 
...
~$[Using a software platform rather than ad hoc solutions is a more robust ]^[ sustain%%dsh%% 
...
~$[Catriona Manville%%lst%% Gavin Cochrane%%lst%% Jonathan Cave%%lst%% Jeremy Millard%%lst%% Jeremy Kevin Pederson%%lst%% Rasmus K are 
...
Athena Vakali%%lst%% Leonidas Anthopoulos%%lst%% ]^[ Srdjan Krco%%per%% 2014%%per%%]$~ ~$[Smart Cities Data Streams Integration%%cln%% Ex%%dsh%% 
