...
the bit%%dsh%%width of the fractional part as F L%%per%%]$~ ~$[We denote a given xed point representation%%lst%% then%%lst%% as 
...
~$[We compare our results to a baseline network trained on the same dataset with the same hyperparame%%dsh%% 
ters%%lst%% ]b[ using full%%dsh%%precision oating point ]f[ all attributes%%per%%]$~ ~$[We also compare against a non%%dsh%%dynamic 
...
using DPS is%%lst%% in general%%lst%% larger than the training loss of the 
...
I%%per%%]$~ ~$[Guyon%%lst%% U%%per%%]$~ ~$[V%%per%%]$~ ~$[Luxburg%%lst%% S%%per%%]$~ ~$[Bengio%%lst%% H%%per%%]$~ ~$[Wallach%%lst%% R%%per%%]$~ ~$[Fergus%%lst%% S%%per%%]$~ ~$[Vishwanathan%%lst%% ]^[ R%%per%%]$~ ~$[Garnett (eds%%per%%)%%lst%% 
