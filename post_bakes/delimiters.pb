...
rapidly%%lst%% at a higher level of abstraction than traditional RTL%%per%%]$~ ~$[For 
...
focused on power benefits rather than performance%%per%% 
...
has more than one call point%%per%%]$~ ~$[Additionally%%lst%% a master interface 
...
~$[Specifically%%lst%% if a start signal is issued by a master interface ]^[ multiple%%dsh%%bit%%dsh%%wide data%%lst%% they cannot be crossed by using a 
...
We compare multi%%dsh%%clock designs to single%%dsh%%clock designs%%lst%% wall%%dsh%%clock time%%per%%]$~ ~$[Ratios larger than 1 indicate a “win” ]f[ 
...
correctness%%per%%]$~ ~$[We verified the multi%%dsh%%domain circuits functioned single%%dsh%%clock design%%lst%% ]^[ hence%%lst%% we use 1 as the ratio ]f[ these 
...
larger than in the inlined case%%lst%% increasing CDC overheads ]^[ 
...
to a single function (and hence a single domain)%%per%%]$~ 
...
]f[ clock%%dsh%%domain assignment%%lst%% rather than altering the programs 
